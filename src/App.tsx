...
import DVLAApp from '../DVLA/src/App';
import PoliceApp from '../Police /src/App';
import { ThemeProvider as DVLAThemeProvider } from '../DVLA/src/contexts/ThemeContext';

function App() {
  const [loggedInApp, setLoggedInApp] = useState<null | 'admin' | 'dvla' | 'police'>(null);
  ...
  const handleNewRegistration = (registrationData: any) => {
    const newApproval = {
    ...
    setPendingApprovals((prev: any[]) => [...prev, newApproval]);
  ...
  if (loggedInApp === 'dvla') {
    return (
      <DVLAThemeProvider>
        <DVLAApp onLogout={() => setLoggedInApp(null)} />
      </DVLAThemeProvider>
    );
  }

  if (loggedInApp === 'police') {
    return <PoliceApp onLogout={() => setLoggedInApp(null)} />;
  }
  ...
  const renderMainContent = () => {
    switch (activeItem) {
      ...
      case 'security':
        return <SecurityManagement onNavigate={setActiveItem} />;
      case 'system-settings':
  ...
  return (
    <div className={`min-h-screen font-['Inter'] flex ${darkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
      ...
      <div className="flex-1 lg:ml-0">
        ...
        <main className={`min-h-[calc(100vh-4rem)] ${darkMode ? 'bg-gray-900' : 'bg-gray-100'} overflow-y-auto`}>
          {renderMainContent()}
        </main>
      </div>
    </div>
